...
class arx
{
    public:
        ...
        std::vector<cipher*> mem_y_new;
        std::vector<cipher*> mem_u_new;
        std::vector<cipher*> mem_y_pre;
        std::vector<cipher*> mem_u_pre;
        ...
        arx()
        {
            this->P_y.resize(4);
            this->Q_u.resize(4);
            this->mem_y_new.resize(4);
            this->mem_u_new.resize(4);
            this->mem_y_pre.resize(4);
            this->mem_u_pre.resize(4);
            this->temp1.resize(4);
            this->temp2.resize(4);
        }
        ...
        ~arx()
        {
            ...
            for(auto& factor : this->mem_y_new)
            {
                if(factor != nullptr)
                {
                    delete factor;
            ...
            this->mem_y_new.clear();

            for(auto& factor : this->mem_u_new)
            {
                if(factor != nullptr)
                {
                    delete factor;
            ...
            this->mem_u_new.clear();

            for(auto& factor : this->mem_y_pre)
            {
                if(factor != nullptr)
                {
                    delete factor;
            ...
            this->mem_y_pre.clear();

            for(auto& factor : this->mem_u_pre)
            {
                if(factor != nullptr)
                {
                    delete factor;
        ...
        void zero_set()
        {
            for(int i = 0; i < 4; i++)
            {
                this->mem_y_new[i] = this->temp->clone();
            }

            for(int i = 0; i < 4; i++)
            {
                this->mem_u_new[i] = this->temp->clone();
            }

            for(int i = 0; i < 4; i++)
            {
                this->mem_y_pre[i] = this->temp->clone();
            }

            for(int i = 0; i < 4; i++)
            {
                this->mem_u_pre[i] = this->temp->clone();
            }
        ...
        void calc()
        {
            for(int i = 0; i < 4; i++)
            {
                crypto_handler::eval_mul(this->P_y[i], this->mem_y_new[i], this->temp1[i]);
                crypto_handler::eval_mul(this->Q_u[i], this->mem_u_new[i], this->temp2[i]);
            }
        ...
        void mem_update(cipher* new_y, cipher* new_u)
        {
            for(int i = 0; i < 4; i++)
            {
                this->mem_y_pre[i]->ciphertext[0]->coeff = this->mem_y_new[i]->ciphertext[0]->coeff;
                this->mem_y_pre[i]->ciphertext[1]->coeff = this->mem_y_new[i]->ciphertext[1]->coeff;
                this->mem_u_pre[i]->ciphertext[0]->coeff = this->mem_u_new[i]->ciphertext[0]->coeff;
                this->mem_u_pre[i]->ciphertext[1]->coeff = this->mem_u_new[i]->ciphertext[1]->coeff;
            }

            for(int i = 0; i < 3; i++)
            {
                std::swap(this->mem_y_new[i], this->mem_y_new[i+1]);
                std::swap(this->mem_u_new[i], this->mem_u_new[i+1]);
            }
            this->mem_y_new[3]->ciphertext[0]->coeff = new_y->ciphertext[0]->coeff;
            this->mem_y_new[3]->ciphertext[1]->coeff = new_y->ciphertext[1]->coeff;
            this->mem_u_new[3]->ciphertext[0]->coeff = new_u->ciphertext[0]->coeff;
            this->mem_u_new[3]->ciphertext[1]->coeff = new_u->ciphertext[1]->coeff;
        }
};
